...
class DocumentVectorizer:
    ...
    def pool_embeddings(self, document_windows_embeddings):
...
argparser.add_argument('--mode', type=str, help='Select mode: cluster(=cluster existing embeddings file)',
                       required=True)
argparser.add_argument('--index-folder', type=str,
                       help='The location of the folder where document embeddings file is stored', required=True)
argparser.add_argument('--cluster-count', type=int, default=10, required=False,
                       help='Create cluster-count clusters for the *existing* document_embeddings.npy file stored in index-folder')
...
if __name__ == "__main__":
    ...
    if mode == "cluster":
        ...
        try:
            ...
            print(
                f"Error: The file '{index_folder}/document_embeddings.npy' does not exist. Please check the path and try again.")
            sys.exit(1)
